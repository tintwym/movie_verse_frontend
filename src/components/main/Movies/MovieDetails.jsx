      ...
      {movie.genres && (
        <div className="movie__details-genre">
          ...
          {movie.genres.map((genre) => {
            ...
            return (
              <Link className="button--key" key={genre.id + genre.name} to={`/genre/${genreName}/${genre.id}`}>
                {genre.name}
              </Link>
            );
          })}
      ...
      {keywords && keywords.length >= 1 && (
        <div className="movie__details-keywords">
          ...
          {keywords.map((keyword) => (
            <Link className="button--key" key={keyword.id + keyword.name} to={`/search/movie/${keyword.name}`}>
              #{keyword.name}
            </Link>
          ))}
        </div>
